...
Given the data to be analysed, we now use the classifyprofile function to calculate enrichment scores between the input profiles and the  data available from the cMap2data package (this can be changed to a users custom data). This data is based on the connectivity map drug screening. Here we use the default options to analyse the data.

<<results=tex>>=
default<-classifyprofile(data=selprofiles$ranklist)
@
...
<<eval=TRUE>>=
default<-classifyprofile(data=sel22528$ranklist)
@

A two sided test is used by DrugVsDisease whereby a gene set of the same size is also generated from the reference data and compared to the input profile.
...
customedge=customedge, customsif=customsif, cytoout=FALSE,signif.fdr=1)
@
