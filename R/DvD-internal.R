...
function(accession,destdir=".",factorvalue=NULL,case=c("disease","drug")){
	...
	if(length(sel)==0&&length(factornames)!=0){
		...
		stop(paste('Neither disease nor dose/drug variable available for data set. 
				   Available factors: ',fn,' use parameter factorvalue to select a factor'))
	}
	
	if(length(sel)==0&&length(factornames)==0){stop('No factornames available in the pData of the eset. 
													Consider processing using the local option.')}
...
.calculateES <-
function(data,type=c("fixed","dynamic","range"),adj=c("qvalue","BH"),lengthtest=250,ranges=seq(100,2000,by=100),pvalues=NULL,dynamic.fdr=0.01,signif.fdr=0.01,customRefDB=NULL,case=c("disease","drug"),noperm=5000){
	case=match.arg(case)
	type=match.arg(type)
	...
	#if a customRefDB is not supplied load the default profiles from either the DrugVsDiseasedata or cMap2data package depending on whether the input profile is a disease or drug profile.
	...
	if(type=="fixed"){
		...
		outk<-.getESscores(rankdata,rankdatainv,drugPRLs=refDB,uplength=lengthtest,downlength=lengthtest)	
		#get permutation based empirical p-values for enrichment scores	
		signif<-.significantES(ESscores=outk,rankdata=rankdata,rankdatainv=rankdatainv,noperm=noperm,fdr=signif.fdr,uplength=lengthtest,downlength=lengthtest,drugPRLs=refDB,adj=adj)
	...
	if(type=="range"){
		...
		for(i in 1:length(ranges)){
			...
			outk[[i]]<-.getESscores(rankdata,rankdatainv,drugPRLs=refDB,uplength=ranges[i],downlength=ranges[i])
			#get permutation based empirical p-values for enrichment scores
			signiftemp<-.significantES(ESscores=outk[[i]],rankdata=rankdata,rankdatainv=rankdatainv,noperm=noperm,fdr=signif.fdr,uplength=ranges[i],downlength=ranges[i],drugPRLs=refDB,adj=adj)			
		...
		if(ncol(pvalues)>1){
			...
			for(i in 1:ncol(pvalues)){
				if(sum(signifgenes[,i])>0){
					...
					outk[[i]]<-.getESscores(rankdata[,i],rankdatainv[,i],uplength=uplength,downlength=downlength,drugPRLs=refDB)
					#get permutation based empirical p-values for enrichment scores
					signif[[i]]<-.significantES(ESscores=outk[[i]],uplength=uplength,downlength=downlength,rankdata=rankdata,rankdatainv=rankdatainv,noperm=noperm,fdr=signif.fdr,drugPRLs=refDB,adj=adj)}
				else{
			...
			if(sum(signifgenes)>0){
			   ...
			   if(uplength<50){
				...
				#calculate enrichment scores
				outk<-.getESscores(rankdata,rankdatainv,drugPRLs=refDB,uplength=uplength,downlength=downlength)	
				#get permutation based empirical p-values for enrichment scores		
				signif<-.significantES(ESscores=outk,rankdata=rankdata,rankdatainv=rankdatainv,noperm=noperm,fdr=signif.fdr,uplength=uplength,downlength=downlength,drugPRLs=refDB,adj=adj)
			}else{
...
.significantES <-
function(ESscores,rankdata=NULL,rankdatainv=NULL,uplength=250,downlength=250,noperm=1000,fdr=0.01,drugPRLs,adj=adj){
	...
	if(is.list(ESscores)){
			...
			scorematrix<-ESscores[[i]]
				...
				if(is.matrix(scorematrix)){
					...
					for(i in 1:nrow(scorematrix)){
      					signifcompounds[[i]]<-.findSignifCompounds(scorematrix[i,],rankdata=rankdata[,i],rankdatainv=rankdatainv[,i],uplength=uplength,downlength=downlength,noperm=noperm,fdr=fdr,adj=adj)
					}
					names(signifcompounds)<-rownames(scorematrix)
				}else{
					signifcompounds<-.findSignifCompounds(scorematrix,rankdata=rankdata,rankdatainv=rankdatainv,uplength=uplength,downlength=downlength,noperm=noperm,fdr=fdr,adj=adj)
				}
		...
		if(is.matrix(scorematrix)){
			...
			signifcompounds[[1]]<-.findSignifCompounds(scorematrix,rankdata=rankdata,rankdatainv=rankdatainv,uplength=uplength,downlength=downlength,noperm=noperm,fdr=fdr,adj=adj)
...
.findSignifCompounds <-
function(scores,rankdata=NULL,rankdatainv=NULL,noperm=100,uplength=250,downlength=250,fdr=fdr,adj=adj){
...
function(ESscores,statistic=c("mean","median"),customClusters=NULL,case=c("disease","drug")){
	...
	if(is.list(ESscores)){
		#then we have done dynamic list of ks
...
.averagecluster <-
function(scores,statistic="mean",customClusters=NULL,case=c("disease","drug")){
	case<-match.arg(case)
	library(DrugVsDiseasedata)
	library(cMap2data)
...
.getESscores <-
function(diseasematrixup,diseasematrixdown,uplength=250,downlength=250,drugPRLs){
...
.combineEnsembl <-
function(data,annotation,genelist){
...
function(annotation,values){
	...
	if(length(sel)==0){stop('Annotation file not available for this platform. Must be one of Affymetrix, HGU133A, HGU133A2 or HGU133Plus2')}
#if(annotation=="hgu133a")require(hgu133a.db)
#if(annotation=="hgu133plus2")require(hgu133plus2.db)
#if(annotation=="hgu133a2")require(hgu133a2.db)
